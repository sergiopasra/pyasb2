...
class CloudCoverage(object):
    ...
    def cloud_coverage(self, StarList_Photom, StarList_woPhot, BouguerFit):
        ...
        for column in xrange(len(self.AZdirs)):
            for line in xrange(len(self.ALTdirs)):
                # Here we should divide /2, /1 will smooth data
                for Star in Stars_in_field:
                    ...
                    if Star.saturated == True:
                        continue
                    elif Star.cold_pixels == True:
                        continue
                    elif Star.masked == True:
                        continue

                    # Star is not saturated and present in the sky, add it
                    # [weighted]
                    ...
                    # Alternative estimation based on flux extinction. To be
                    # completed.
                    Predicted_Flux      = \
                        10 ** (0.4 *
                               (BouguerFit.Regression.mean_zeropoint - Star.FilterMag))
                    Predicted_FluxError =\
                        Predicted_Flux * \
                        np.log(10) * 0.4 * \
                        BouguerFit.Regression.error_zeropoint
                    Measured_Flux = Star.starflux
                    Measured_FluxError = Star.starflux_err

                    PercentageFlux[line][
                        column] += np.clip(Measured_Flux / Predicted_Flux, 0, 1)

                # If there are not enough stars in the field, truncate the
                # measure
        ...
        self.CloudCoverageErr = self.cloud_coverage_error(
    ...
    def cloud_map(self, BouguerFit, ImageInfo):
        ...
        def ticks_and_locators():
            ...
            self.Cloudgraph.set_thetagrids(
                theta_locator, theta_label, size="large")
        ...
        def color_bar():
            ...
            self.Cloudcolorbar.set_label(
                "Cloud Coverage", rotation="vertical", size="large")
        ...
        self.Cloudgraph.text(
            5 * np.pi / 4, 145, unicode(image_information, 'utf-8'), fontsize='x-small')
