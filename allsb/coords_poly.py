...
def main(argv=None):
    ...
    print('load region file', region_name)
    ...
    print('read catalog', catfile)
    table = Table.read(catfile, format='ascii.csv')

    print('join on names (region with catalog)')
    table_obs = astropy.table.join(table, table_name)
    print('done')

    print('convert coordinates to AltAz')
    #print(table)
    coords_sky = SkyCoord(
        ra=table_obs['raj1950'],
        dec=table_obs['dej1950'],
    ...
    # star position predictions
    coords_altz = coords_sky.transform_to(aaframe)
    print('add AltAz columns to table_obs')
    table_obs.add_column(Column(coords_altz.alt.radian, name='alt'))
    table_obs.add_column(Column(coords_altz.az.radian, name='az'))
    table_obs.add_column(Column(coords_altz.alt.degree, name='alt_deg'))
    table_obs.add_column(Column(coords_altz.az.degree, name='az_deg'))
    ...
    if False:
        ...
        plt.xlim([0, 3866])
        plt.ylim([0, 2574])
        ax.scatter(coords_x, coords_y, label='measured')
        ax.scatter(x2[:,0], x2[:,1], label='predicted (initial)')
        plt.legend()
        plt.show()
    ...
    if False:
        ...
        axs.scatter(nom_phi, 90 - np.rad2deg(nom_theta), label='nominal')
        axs.set_rmax(90)
        plt.legend()
        plt.show()
    ...
    ang_theta, ang_phi = compute(coords_x, coords_y, xcenter, ycenter, rad_scale, az=az2_pole)
    ...
    if False:
        ...
        axs.set_theta_direction(-1)
        axs.scatter(nom_phi, 90 - np.rad2deg(nom_theta), label='nominal')
        axs.scatter(ang_phi, 90 - np.rad2deg(ang_theta), label='computed')
        axs.set_rmax(90)
        plt.legend()
        plt.show()
    ...
    join = np.array([coords_x, coords_y]).T
    coord_ang1 = wcs1.wcs_pix2world(join, 0)
    ...
    if False:
        ...
        axs.set_theta_direction(-1)
        axs.scatter(nom_phi, 90 - np.rad2deg(nom_theta), label='nominal')
        axs.scatter(ang_phi, 90 - np.rad2deg(ang_theta), label='computed0')
        axs.scatter(np.deg2rad(coord_ang1[:,1]), 90 - coord_ang1[:,0], label='computed1')
        axs.set_rmax(90)
        plt.legend()
        plt.show()

    #plt.scatter(sub2['az_deg'],sub2['alt_deg']- coord_ang1[:,0])
    #
    if False:
        plt.ylim([-1, 1])
        plt.scatter(sub2['az_deg'], coord_ang1[:,1] - sub2['az_deg'])
        plt.show()
        plt.ylim([-1, 1])
        plt.scatter(sub2['alt_deg'], coord_ang1[:, 1] - sub2['az_deg'])
        plt.show()

        plt.scatter(sub2['alt_deg'], coord_ang1[:,0] - sub2['alt_deg'])
        plt.show()
...
def compute_poly(x, y, r0, r1, rad, az=0.0):
    # Returns radians
    # radial_factor = 1 / 14.19766968
    radial_factor = rad
    pr0 = np.asarray(x) - r0
    ...
    factor = 1 - np.rad2deg(rad_theta) ** 2 / 2.0
    ang_theta = np.arcsin(factor)
