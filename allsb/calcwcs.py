...
def compute_wcs(image_info, catalog):
    #
    ...
    rad0 = 1750
    # x, y = 2755, 1830
    y, x = np.divide(image_info['shape'],2)
    res = (x, y, math.sqrt(2) / rad0)

    table = prepare_astrometry_catalogue(catalog, altaz_frame, min_altitude)
    _logger.debug('we have %s stars for astrometry', len(table))

    wcs_catalog_filename = image_info['wcs_catalog']
    table_init = calc_obstable(image_info, wcs_catalog_filename, altaz_frame, do_plot=False)
    print('resolutionpars', res)
    val0 = solve_plate_zen_eqa0(table_init, x0=res[0], y0=res[1], scale=res[2])
    ...
    # Object detection
    data = fits.getdata(image_info['filename'])
...
#    plt.colorbar()
    ...
    with fits.open(image_info['filename']) as hdul:
    ...
    if plot_p2:
        ...
        min_altitude = 20
        table2 = prepare_phot_catalogue(table2, altaz_frame, min_altitude)
    ...
    table3 = prepare_phot_catalogue(table3, altaz_frame, min_altitude)
